...
class Product:
    ...
    def __init__(self,root):
        ...
        #create object reference instance of Databae class as p:
        p = Database()
        p.conn()
        ...
        '''Lets cal the database methods to perform database operations'''
        # fucton to close the product frame
        def close():
            print("Product : close method called ")
            close = tkinter.messagebox.askyesno("WAREHOUSE INVENTORY \
                SALES PURCHASE MANAGEMENT SYSTEM","Really.... Do you want \
                    to close the system")
            if close > 0:
                root.destroy()
        ...
        def insert():
            ...
            if ( len(pId.get())!=0):
                ...
                productList.insert(END,pId.get(),pName.get(),pQty.get(),
                        pPrice.get(),pCompany.get(),pContact.get())
        ...
        #functon responsible to show product table data to scroll productlist    
        ...
        self.buttonShowData = Button( OperationFrame, text='Show Data',
            font=('arial',18,'bold'),height=1,width='10',
...
class Database:
    ...
    def search(self,pid="",name="",price="",qty="",company="",contact=""):
        ...
        cur.execute("select * from product where pid=? or pname=? or \
            price=? or qty=? or company=? \
            or contact=?",(pid,name,price,qty,company,contact))
        row=cur.fetchall()
        con.close()
    ...
    def update(self,pid="",name="",price="",qty="",company="",contact=""):
        ...
        qty=? or company=? or contact=? where pid=?",(pid,name,price,qty,
                                        company,contact,pid))
        con.commit()
        con.close()
